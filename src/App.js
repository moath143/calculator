...
import A from './constants/ActionsType';
import I from './constants/IntegerFormatter';
import R from './constants/Reducers';
...
export default function App() {
  ...
  return (
    <div className="calculator-grid">
      ...
      {/* ROW 2 */}
      <button className="span-two" onClick={actions.clear}>
        AC
      </button>
      ...
      <button className="span-two" onClick={actions.evaluate}>
        =
      </button>
    </div>
  );
...
const mapDispatch = (dispatch) => ({
  ...
  chooseOperation: (operation) =>
    dispatch({ type: A.CHOOSE_OPERATION, payload: { operation } }),
...
function formatOperand(operand) {
  ...
  if (decimal == null) return I.format(integer);
